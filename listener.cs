...
namespace ACFramework
{
    ...
    //new listener class (tutorial 3) for making Yoshi run when the up and down arrows are pressed
    class cListenerQuakeScooterYHopper : cListenerScooterYHopper
    {
        public cListenerQuakeScooterYHopper(float walksp, float hopstr) : base(walksp, hopstr)
        {
        ...
        public override void listen(float dt, cCritter pcritter)
        {
            ...
            if (!_hopping && up)
            {
                pcritter.Velocity = pcritter.AttitudeTangent * pcritter.MaxSpeed / 3;
